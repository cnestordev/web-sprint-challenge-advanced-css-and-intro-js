...
artists.forEach((artist) => {
  if (artist.name === 'Vincent van Dough') {
    ...
    console.log('successfully changed aritst name', artist);
  }
});
...
function getArtistByIndex(array, index) {
  const artistObj = array[index];
  return `The artist at index ${index} is ${artistObj.name}`;
}
...
function get20s(array) {
  const artists20 = array.filter((artist) => {
    const yearLived = Number(artist.years.slice(-4));
    if (yearLived < 2000 && yearLived >= 1900) {
      return yearLived;
    }
  });
  return artists20;
...
function lotsOfArt(arr) {
  const lotsOfArtArr = arr.filter((artist) => {
    let numberOfArt = Number(artist.paintings);
    if (numberOfArt > 100) {
      return artist;
    }
  });
...
function randomize(arr) {
  ...
  while (clonedArr.length > 0) {
    ...
    let deletedItem = clonedArr.splice(randomIndex, 1);
    newArr.push(deletedItem[0]);
  }
  return newArr;
}
